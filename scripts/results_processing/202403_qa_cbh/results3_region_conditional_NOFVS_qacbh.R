...
input_folder <- file.path("qa", "qa_cbh")
output_folder <- file.path("qa", "qa_cbh")
...
hacbp <- cbp %>% 
  group_by(HUC12, Region, Priority, TxIntensity, TxType, run, Year, mas_scenario, cbh_kept) %>% 
...
hacft_hist <- cft_hist %>% 
  group_by(HUC12, Region, Priority, TxIntensity, TxType, run, Year, mas_scenario, fire_type, cbh_kept) %>% 
...
ft <- hacft_hist %>%
  ...
  pivot_wider(
    id_cols = c(HUC12, Region, Priority, TxIntensity, TxType, run, Year, mas_scenario, cbh_kept),
    names_from = type,
...
combined <- hacbp %>% 
  left_join(hacfl, 
            by = join_by(HUC12, Region, Priority, TxIntensity, TxType, 
                         run, Year, mas_scenario, cbh_kept)) %>% 
  left_join(ft,
            by = join_by(HUC12, Region, Priority, TxIntensity, TxType, 
                         run, Year, mas_scenario, cbh_kept))
...
# IMPLICIT
# this check doesn't work with most test runs, make sure to check with what makes sense
