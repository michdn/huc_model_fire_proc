...
gf_bl <- gf %>% 
  ...
  dplyr::select(HUC12, Year, HaCFL, HaCBP, surface, passive_crown, active_crown) %>% 
...
gf_b2 <- gf %>% 
  ...
  #filter to treatment year. (yes, this drops WUI 4th, and ignore WUI retreat)
  filter(Year == trt_yr) %>% 
  #only 2m 
  filter(TxIntensity == "2m") %>% 
  #join with bl for single row
  left_join(gf_bl, by = join_by(HUC12, Year)) %>% 
...
bp_b2 <- bp %>% 
  ...
  filter(TxIntensity == "2m") %>% 
  #join with bl for single row
  left_join(bp_bl, by = join_by(HUC12, Year)) %>% 
  #calculate differences (base - 2m)
...
# FUEL ADJ ROS and FL
# Already have baseline info in rows 

adj_fl_b2 <- adj_fl %>% 
  filter(Year == trt_yr) %>% 
  filter(TxIntensity == "2m") %>% 
  mutate(adj_fl_pc = (mean_bl - mean) / mean_bl) %>% 
  dplyr::select(HUC12, Priority, TxIntensity, TxType, Year,
...
adj_ros_b2 <- adj_ros %>% 
  filter(Year == trt_yr) %>% 
  filter(TxIntensity == "2m") %>% 
  mutate(adj_ros_pc = (mean_bl - mean) / mean_bl) %>% 
  dplyr::select(HUC12, Priority, TxIntensity, TxType, Year,
...
b2 <- gf_b2 %>% 
  left_join(bp_b2, by = join_by(HUC12, Priority, TxIntensity, TxType, Year)) %>% 
  left_join(adj_fl_b2, by = join_by(HUC12, Priority, TxIntensity, TxType, Year)) %>% 
  left_join(adj_ros_b2, by = join_by(HUC12, Priority, TxIntensity, TxType, Year)) %>% 
...
for (p in seq_along(priorities)){
  ...
  p1a <- ggplot() + 
    ...
    labs(title="Gridfire HaCFL vs BehavePlus FL", 
         x="GF HaCFL Base-2m %change",
         y="BP FL Base-2m %change") +
    geom_label(data=t_r,
              mapping=aes(label=count, x=0.5*max_x, y=max_y), 
  ...
  p1a2 <- ggplot() + 
    ...
    labs(title="Gridfire HaCFL/HaCBP vs BehavePlus FL", 
         x="GF HaCFL/HaCBP Base-2m %change",
         y="BP FL Base-2m %change") +
    geom_label(data=t_r,
  ...
  p2a <- ggplot() + 
    ...
    labs(title="Gridfire HaCFL vs Fuel Adjective FL", 
         x="GF HaCFL Base-2m %change",
         y="Fuel Adj FL Base-2m %change") +
    geom_label(data=t_r,
               mapping=aes(label=count, x=max_x*0.5, y=max_y),              
               size=3, hjust="inward", alpha=0.5) +
    geom_label(data=b_r,
               mapping=aes(label=count, x=max_x, y=min_y),
               size=3, hjust="inward", alpha=0.5) +
  ...
  p1b <- ggplot() + 
    ...
    labs(title="Gridfire HaCBP vs BehavePlus ROS", 
         x="GF HaCBP Base-2m %change",
         y="BP ROS Base-2m %change") +
    geom_label(data=t_r,
               mapping=aes(label=count, x=max_x*0.5, y=max_y),
               size=3, hjust="inward", alpha=0.5) +
    geom_label(data=b_r,
  ...
  #collect all
  all_title <- paste0("Scatter plots at Year of (Initial) Treatment: Percent Change (fraction) 2m to Baseline (base-2m): ", this_priority)
  ...
  fn_p <- paste0("scatters_gf_bp_fa_", this_priority, "_v2.jpg")
