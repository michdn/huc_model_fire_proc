...
#use gridfire results file to get timing groups
...
res <- bind_rows(ress, res_bases) %>% 
  #add in timing
  left_join(timing %>% 
              dplyr::select(HUC12, Priority, timing_group), 
            by = join_by(HUC12, Priority))
...
for (r in seq_along(regions)){
  ...
  #output 
  plot_folder <- file.path('plots', 'timeseries_behaveplus', this_reg)
  ...
  for (p in seq_along(priorities)){
    ...
    for (t in seq_along(trts)){
      ...
      if (graph_boxplots) {
        #plot 1 : boxplot all HUCs,  timeseries
        p1 <- ggplot() +
          geom_boxplot(data = res_r_p_t,
                       mapping = aes(x = Year, y = wtd_avg_FL, group=Year)) +
        ...
        fn1 <- paste0(this_reg, '_', this_priority, '_', this_trt, '_',
                      'boxplot_', box_scales, '_behaveplusFL.jpg')
        ggsave(plot = p1,
        ...
        p2 <- ggplot() +
          geom_boxplot(data = res_r_p_t,
                       mapping = aes(x = Year, y = wtd_avg_ROS, group=Year)) +
          scale_x_continuous(breaks = year_breaks) +
          facet_wrap(~TxIntensity+timing_group, 
                     scales = box_scales, 
                     ncol = trt_cols) +
        ...
        fn2 <- paste0(this_reg, '_', this_priority, '_', this_trt, '_',
                      'boxplot_', box_scales, '_behaveplusROS.jpg')
        ggsave(plot = p2,
        ...
        p3 <- ggplot() +
          geom_boxplot(data = res_r_p_t,
                       mapping = aes(x = Year, y = wtd_avg_type, group=Year)) +
          scale_x_continuous(breaks = year_breaks) +
          facet_wrap(~TxIntensity+timing_group, 
                     scales = box_scales, 
                     ncol = trt_cols) +
        ...
        fn3 <- paste0(this_reg, '_', this_priority, '_', this_trt, '_',
                      'boxplot_', box_scales, '_behaveplusType.jpg')
        ggsave(plot = p3,
      ...
      if(graph_original_sampled){
        ...
        #plot 6: sampled line,  timeseries
        p6 <- ggplot(data = res_r_p_t_sample,
                     mapping = aes(x=Year, y=wtd_avg_FL, color=TxIntensity)) +
        ...
        fn6 <- paste0('sampled_', this_reg, '_', this_priority, '_', this_trt, '_',
                      'line_freey_', 'behaveplusFL.jpg')
        ggsave(plot = p6,
               filename = file.path(sampled_folder, fn6),
               width = 7, height = 7, units = 'in')
        
        p7 <- ggplot(data = res_r_p_t_sample,
                     mapping = aes(x=Year, y=wtd_avg_ROS, color=TxIntensity)) +
          geom_jitter(shape = 1, height = 0, width = 0.3) +
          geom_line() +
          scale_x_continuous(breaks = year_breaks) +
          scale_color_manual("Intensity", values = i_colors) + 
        ...
        p8 <- ggplot(data = res_r_p_t_sample,
                     mapping = aes(x=Year, y=wtd_avg_type, color=TxIntensity)) +
          geom_jitter(shape = 1, height = 0, width = 0.3) +
          geom_line() +
          scale_x_continuous(breaks = year_breaks) +
          scale_color_manual("Intensity", values = i_colors) + 
        ...
        fn8 <- paste0('sampled_', this_reg, '_', this_priority, '_', this_trt, '_',
                      'line_freey_', 'behaveplusType.jpg')
        ggsave(plot = p8,
      ...
      if(graph_new_sampled){
        ...
        #plot 11: sampled line, timeseries
