...
res_orig <- read_csv(file.path("..", "..",
  ...
  filter(Region %in% c("SC", "CC"),
         !Priority %in% c("baseline", "baseweather"))
...
Note that this is collapsing all HUCs (averaging) so we may not be seeing the variability of the HUCs within a scenario. We also may want to factor in treatment timing in more detailed analyses as well.  
...
In this case, we will look at a VERY simplified example of the average flame index (expFlame) over all years (2024, 2029, 2034, 2039). We will group our data by HUC and scenario (and notably not Year), then summarize to calculate the mean of expFlame. Then we arrange our dataset by the mean of expFlame and take the first (lowest) value per HUC12. This leaves us with a single row per HUC of the scenario that produced the lowest flame index for that HUC. 
