...
res_orig <- bind_rows(res_sc, res_cc)

#hidden section to update current interim datacube to what will be in final
hucs_timing <- st_read("../../data/data_huc/TxHucsTimingGroups.shp")
res_orig <- res_orig %>%
  left_join(hucs_timing %>%
              st_drop_geometry() %>%
              select(huc12,
...
#read in data
res_orig <- read_csv("datacube.csv") %>% 
  mutate(HUC12 = as.character(HUC12))
...
plot_yr2039_sc_expFlame_pt <- ggplot() + 
  ...
  stat_smooth(data=res2039sc,
              ...
              se = FALSE, 
              fullrange = TRUE) + 
  labs(title="SC: Year 2039: Expected Total Flame Index",
       x="500k Acres Treated",
       y="2.3 Million Acres Treated") +
  facet_wrap(~Priority+TxType)
...
ggplot() +
  geom_boxplot(data=r39sc,
    ...
    labs(title="SC: Year 2039: Expected Total Flame Index per HUC",
       x="Intensity - Treatment Type",
...
The above examples were exploring a timeslice, a single year. We may also want to look at the results over time. Let's select SC as our region, 'Fire' as the priority, and 'trt4' (heavy thinning without removal) as the treatment type. We'll look at the three intensities over the four years. 
...
#Set timing group, the if/else is useful if you are working inside a loop
if (this_priority == "Fire"){
  res_example <- res_example %>% 
    mutate(timing_group = timeFire) 
} else if (this_priority == "WUI"){
  res_example <- res_example %>% 
    mutate(timing_group = timeWUI)
} else if (this_priority == "Hybrid"){
  res_example <- res_example %>% 
    mutate(timing_group = timeHybrid) 
} else {
...
We'll plot boxplots with the year along the x-axis and different metrics on the y-axis. The graphs will facet on intensity and timing group. 
...
We can also look at a timeseries of the average of all HUCs (irrespective of timing group). In the example below, we've selected a region and treatment type, and will show timeseries linegraphs by priority (color) and intensity (line type). This way, we can somewhat compare scenarios of the same treatment type against each other. 
...
Note that this is collapsing all HUCs (averaging) so we may not be seeing the variability of the HUCs within a scenario. 
...
In this case, we will look at the average flame index (expFlame) over all years (2024, 2029, 2034, 2039). We will group our data by HUC and scenario (and notably not year, unlike in other places), then summarize to calculate the mean of expFlame. Then we arrange our dataset by the mean of expFlame and take the first (lowest) value per HUC12. This leaves us with a single row per HUC of the scenario that produced the lowest flame index for that HUC. 
...
We can map using R, as in the example below, but you may also wish to save our your new spatial dataset to map in QGIS, ESRI ArcGIS, or other GIS software. 
...
#Here is how you could save out the dataset to use in another program. 
