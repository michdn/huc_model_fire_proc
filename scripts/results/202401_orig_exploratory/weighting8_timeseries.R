...
#output 
plot_folder <- file.path('plots', 'timeseries', 'freey')
...
#                mapping = aes(x = Year, y = HaCFL, group=Year)) +
#   scale_x_continuous(breaks = year_breaks) +
#   facet_wrap(~TxIntensity+fireGrpLbl) + 
#   labs(title = "SC Fire trt1")
# 
# 
# ggplot() +
#   geom_boxplot(data = rsc %>%
...
#                  filter(Priority == "Fire",
#                         TxType == "trt4"),
#                mapping = aes(x = Year, y = HaCFL, group = HUC12)) +
#   scale_x_continuous(breaks = year_breaks) +
...
#               filter(Priority == "Fire",
#                      TxType == "trt4"),
#             mapping = aes(x = Year, y = expFlame/hucAc, group = HUC12)) +
#   scale_x_continuous(breaks = year_breaks) +
#   facet_wrap(~TxIntensity+fireGroup) + 
#   labs(title = "SC Fire trt4")
# 
...
#             mapping = aes(x = Year, y = HaCFL, group = HUC12)) +
#   scale_x_continuous(breaks = year_breaks) +
#   facet_wrap(~fireGroup) + 
#   labs(title = "SC Fire 2m trt4")
# 
# ## SC sampled hucs ----------------------------------------------------
# 
# # fire 
# 
# fire_hucs <- hucs_sample %>% 
#   filter(priority == "fire") 
# 
# rsc_smp_fire <- rsc %>% 
#   filter(HUC12 %in% fire_hucs$HUC12) 
# 
# 
# ggplot(data = rsc_smp_fire %>% 
#          filter(Priority == "Fire",
#                 TxType == "trt4"),
#        mapping = aes(x=Year, y=expFlame, color=TxIntensity)) + 
#   geom_jitter(shape = 1, height = 0, width = 0.3) +
#   geom_line() + 
#   scale_x_continuous(breaks = year_breaks) + 
#   facet_wrap(~paste0(fireGrpLbl, " ", HUC12),
#              ncol = 2) + 
#   labs(title = "SC Fire trt 4: Selected HUCs")
# 
# 
# ggplot(data = rsc_smp_fire %>% 
#          filter(Priority == "Fire",
...
#   facet_wrap(~paste0(fireGrpLbl, " ", HUC12),
#              ncol = 2) + 
#   labs(title = "SC Fire trt 4: Selected HUCs fireGroup 25 & 50")
# 
# ggplot(data = rsc_smp_fire %>% 
#          filter(Priority == "Fire",
#                 TxType == "trt4",
#                 fireGroup == 75 | fireGroup == 100),
#        mapping = aes(x=Year, y=expFlame, color=TxIntensity)) + 
#   geom_jitter(shape = 1, height = 0, width = 0.3) +
#   geom_line() + 
#   scale_x_continuous(breaks = year_breaks) + 
#   facet_wrap(~paste0(fireGrpLbl, " ", HUC12),
#              ncol = 2) + 
#   labs(title = "SC Fire trt 4: Selected HUCs fireGroup 75 & 100")
# 
# # rsc_smp_fire %>% 
# #   filter(HUC12 == 181002010703,
# #          Priority == "Fire",
# #          TxType == "trt4") %>% View()
# 
# ggplot(data = rsc_smp_fire %>% 
...
#   facet_wrap(~paste0(fireGrpLbl, " ", HUC12),
#              ncol = 2) + 
#   labs(title = "SC Fire trt 6: Selected HUCs")
# 
# ggplot(data = rsc_smp_fire %>% 
#          filter(Priority == "Fire",
#                 TxType == "trt6",
...
#   facet_wrap(~paste0(fireGrpLbl, " ", HUC12),
#              ncol = 2) + 
#   labs(title = "SC Fire trt 6: Selected HUCs fireGroup 100")
# 
# 
# 
# # HaCFL
