...
-results4_region_absolutemetrics.R: For a specific region, this script uses the annual burn probability raster layer to transform the conditional metrics into absolute metrics. This produces a regional mini datacube. 
-results5_datacube.R: Simply appends the four regional absolute metric mini datacubes into one datacube. Adds any finishing touches. 
...
-timeseries_treatmentgroups.R: Boxplot timeseries, the ones David Saah originally asked for on one of the early calls. Facetted - each row-set is an intensity (500k/1m/2m), and each column-set is a timing group (e.g. treated in 2024, 2029, 3034, 2039). The x-axis are years, and y-axis is the metric (multiple metrics). Script loops for all regions, all priorities, all treatment types. Each plot is one region-priority-trt over time. 
-timeslice_treatmentgroups.R: Boxplot of fire metric (multiple) by treatment intensity (x-axis) for a particular year/epoch. Facetted by treatment type (row-sets) and by treatment timing (column-sets). 


scripts/qa_misc: Miscellaneous one-off scripts, or some qa checks
